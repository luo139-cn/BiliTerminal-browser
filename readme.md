> **本仓库声明**：本项目 fork 自 [Rob​inNotBad/BiliClient（哔哩终端）](https://gitee.com/RobinNotBad/BiliClient)，原作者保留一切权利。本仓库仅在此基础上添加了「内置浏览器」功能（将外链在 App 内用 WebView 打开），并配置了 GitHub Actions 云端自动编译。其余代码与功能均未改动。
